...
- schema version `1.0.1` is returned;
...
Version 1.0.1 records these markers but `v_pgss_delta` does not yet enforce all
...
- A PostgreSQL restart or statistics reset must split report intervals. Version
  1.0.1 records these markers, but consumers must compare them before using
  cumulative deltas.
