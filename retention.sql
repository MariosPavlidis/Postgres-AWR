...
DECLARE v_deleted bigint; v_wait_deleted bigint;
BEGIN
  ...
  WITH gone AS (
    DELETE FROM dba_mon.wait_sample w
    USING dba_mon.cluster_target c
    WHERE w.cluster_id=c.cluster_id
      AND (p_cluster_id IS NULL OR c.cluster_id=p_cluster_id)
      AND w.sampled_at < clock_timestamp()-c.detail_retention
    RETURNING 1
  ) SELECT count(*) INTO v_wait_deleted FROM gone;
  RAISE NOTICE 'purged % snapshots',v_deleted;
  RAISE NOTICE 'purged % wait samples',v_wait_deleted;
END $$;
